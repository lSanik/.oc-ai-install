...
Під час блоку 9 інсталяції — задай питання:
...
- Після `<?php` — перший виконуваний рядок `die(0);` / `die();`; **другий рядок завжди** (порожній або коментар)
- Лише **факти** змін архітектури БД: поля, таблиці, індекси, `ALTER` тощо — для розуміння, що змінити на проді
- Повна модель схеми для ШІ ведеться в **`db_mapping.md`**; при зміні схеми оновлюють обидва артефакти
