...
| `migration.php` | Журнал ручних змін схеми БД для проду; після `<?php` — `die(0);`/`die();`, завжди 2-й рядок (порожній або коментар). Модель БД для ШІ — у `db_mapping.md`. | Не редагувати без явної команди; читати для контексту. |
...
### migration.php
Файл у корені — **журнал змін схеми БД для людини** (що накатити на прод). **Першоджерело моделі БД для ШІ** — `db_mapping.md` у каталозі обраного TOOL (`.claude/` або `.cursor/`). При зміні схеми оновлюй обидва файли.
...
4. Оновлюється через `/update_ai_doc` після кожної задачі яка змінює структуру
5. Всі посилання на AI-файли — з повним шляхом: `.cursor/ai-task.md`, `.cursor/db_mapping.md` або `.claude/ai-task.md`, `.claude/db_mapping.md` — залежно від `TOOL`
